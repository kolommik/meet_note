...
def format_analysis_results(analysis_results: Dict[str, Any]) -> str:
    """
    Форматирует результаты анализа для отображения в пользовательском интерфейсе.

    Args:
        analysis_results: Результаты анализа от identify_speakers_with_llm

    Returns:
        str: Отформатированный текст для отображения
    """
    return safe_operation(
        _format_analysis_results_impl,
        ErrorType.UNKNOWN_ERROR,
        analysis_results=analysis_results,
        default_return="Ошибка при форматировании результатов анализа",
    )


def _format_analysis_results_impl(analysis_results: Dict[str, Any]) -> str:
    """Внутренняя реализация форматирования результатов."""
    if "error" in analysis_results:
        return f"Ошибка анализа: {analysis_results['error']}"

    formatted_text = "## Результаты анализа транскрипции\n\n"

    # Добавляем общее описание
    if "summary" in analysis_results:
        formatted_text += f"### Тема разговора\n{analysis_results['summary']}\n\n"

    # Добавляем информацию о спикерах
    formatted_text += "### Информация о спикерах\n\n"

    for speaker_id, speaker_info in analysis_results.get("speakers", {}).items():
        formatted_text += f"**{speaker_id}**\n"
        formatted_text += f"- Имя: {speaker_info.get('name', 'Неизвестно')}\n"
        formatted_text += f"- Роль: {speaker_info.get('role', 'Не определена')}\n"
        formatted_text += (
        ...
        if "statistics" in speaker_info:
            ...
            formatted_text += f"- Слов: {stats.get('word_count', 'Н/Д')}\n"
            formatted_text += f"- Доля в разговоре: {stats.get('percentage', 'Н/Д')}%\n"
            formatted_text += f"- Высказываний: {stats.get('utterances', 'Н/Д')}\n"
